...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if($stmt = mysqli_prepare($db, "INSERT INTO User(email, name, pass) VALUES(?, ?, ?);")){
       
        mysqli_stmt_bind_param($stmt, "sss", $entered_email, $entered_username, $entered_password);
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            //controlling 
            if(mysqli_stmt_num_rows(mysqli_stmt_execute($db, "SELECT * FROM User WHERE email = $_POST['email']")) == 1){
...
<script type="text/javascript">
    function check() {
        ...
        if (usernameVal !== "" && passwordVal !== "" && userEmail !== "") {
           var form = document.getElementById("registerForm").submit();
        }
        else {
        alert("either password or username or Email or both are empty!");
